...
parser.add_argument('--output_dir', type=str,default= 'ckps/target_salt/')
...
def get_subspace(feats,args,name='source'):
    feat_dir = args.output_dir_src
    subspace_file = os.path.join(feat_dir,name+'_subspace_'+str(args.n_components)+'_'+str(args.standardize)+'.pkl')
...
# print(dir_root)
def call_acc_ece(dataloader,model_f,model_c,update_M_model,epoch,name='target_train'):
    with torch.no_grad():
        model_f.eval()
        model_c.eval()
        loss_total = 0
        ...
        for batch_idx, (data, label,
            ...
            features_test = torch.mm(features_test,W)
...
def main():
    ...
    source_feats = feat_load(args,'source')['all_F_feats']
    target_feats = feat_load(args,'target')['all_F_feats']
    
    source_subspace = get_subspace(source_feats,args,'source')
    target_subspace = get_subspace(target_feats,args,'target')
    initial_m=torch.mm(target_subspace.T,source_subspace)
    xa=torch.mm(torch.mm(target_subspace,target_subspace.T),source_subspace)
 
    initial_w=torch.mm(xa,source_subspace.T)
    update_M_model = Model.update_M(args.n_components,source_subspace,target_subspace,initial_m)
    ...
    for k,v in update_M_model.named_parameters():
        param_group += [{'params': v, 'lr': args.lr }]
    ...
    for epoch in range(max_epoch):
        ...
        for batch_idx, batch_s in enumerate(target_train_dataloader):
            ...
            classifier_loss = 0.025*hlr(outputs)
            ...
            if args.sa_loss_par>0:
                loss_subspace=args.sa_loss_par * F.mse_loss(source_subspace, torch.mm(target_subspace,M))


            classifier_loss += loss_subspace
        ...
        target_train_acc_ece_dict = call_acc_ece(target_train_dataloader_no_aug,model_f,model_c,update_M_model,epoch,name='target_train')
        print(target_train_acc_ece_dict['log_str'])
        target_test_acc_ece_dict = call_acc_ece(target_test_dataloader,model_f,model_c,update_M_model,epoch,name='target_test')
        ...
        if full_target_acc > best_target_full_acc:
            ...
            torch.save(update_M_model.state_dict(), osp.join(args.output_dir,'model_M_adapted.pt'))
...
if __name__ == '__main__':
    ...
    for task_sameS in task_all:
        ...
        if not osp.exists(path_task):
            os.mkdir(path_task)'''
    args.output_dir= args.output_dir+'_lr'+str(args.lr)+'_std_'+str(args.standardize)+'_d_'+str(args.n_components)+\
        '_sa_loss_'+str(args.sa_loss_par)+'_ent_loss_'+str(args.ent_par)
