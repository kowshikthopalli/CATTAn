...
#add output_dir
parser.add_argument('--output_dir', type=str,default= 'models_with_bn')
...
def main():
    ...
    source_test_dataloader = DataLoader(source_test_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=8, drop_last=True)
    target_train_dataloader1 = DataLoader(target_train_dataset1, batch_size=BATCH_SIZE, shuffle=True, num_workers=8, drop_last=True)
    target_test_dataloader1 = DataLoader(target_test_dataset1, batch_size=BATCH_SIZE, shuffle=True, num_workers=8, drop_last=True)
    ...
    def obtain_features_subspace(loader,netF, netC):
        all_F_feats=[]
        all_C_ops =[]
        all_labels=[]
        with torch.no_grad():
            ...
            for i in range(len(loader)):
                ...
                outputs = netC(F_feats)
                all_F_feats.append(F_feats.data.cpu().numpy())
                all_C_ops.append(outputs.data.cpu().numpy())
                all_labels.append(labels.data.cpu().numpy())
        all_F_feats = np.concatenate(all_F_feats, axis=0)
        print('len(all_F_feats)',len(all_F_feats))
        all_C_ops = np.concatenate(all_C_ops, axis=0)
        all_labels = np.concatenate(all_labels, axis=0)

        basis =None
        
        return {'all_F_feats':all_F_feats,'all_C_outputs':all_C_ops,'labels':all_labels}
    ...
    for epoch in range(max_epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, label,
                            idx) in enumerate(source_test_dataloader):
                data = data.to(device=device)
                label = label.to(device=device).long()
            ...
            if pred_acc_s > best_source_test_acc:
                ...
                print('saving with best source test acc:',best_source_test_acc)
                torch.save(best_netF,args.output_dir+"/F_src.pt")
                torch.save(best_netC, args.output_dir+"/C_src.pt")
                #also lets save the feats 
                source_train_return_dict = obtain_features_subspace(source_train_dataloader, model_f,model_c)
                
                source_train_return_dict = obtain_features_subspace(target_train_dataloader1, model_f,model_c)
                pickle.dump(source_train_return_dict, open(args.output_dir+'/source_feats_labels.pkl', 'wb'))
                pickle.dump(source_train_return_dict, open(args.output_dir+'/target_feats_labels.pkl', 'wb'))
